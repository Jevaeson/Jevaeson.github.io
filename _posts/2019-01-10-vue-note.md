...
```vue
methods: { submitComment () { console.log('submitComment') } }
```
...
```vue
export default { name: 'CommentBox', data: () => ({ message: '' }), methods: {
submitComment() { console.log('submitComment', this.message) this.message = '' }
} }
```
...
```vue
this.comments = [ ...this.comments, { id: (this.comments.length + 1).toString(),
body: this.msg } ]
```
...
```vue
computed: { reversedComments: function () { return
this.comments.slice().reverse() } },
```
